...
本節ではまず，微分が関数の最小化に役立つことを，具体例を用いて直感的に理解しましょう。
例として、下図のような下向きにくぼんだ形をした関数がどこで最小値をとるかを探す問題を考えます。


![微分1](../figures/source/04-basics-of-differential/03_02.png)

適当な点 $\theta_{1}$ でこの関数のグラフに接する直線（**接線**）を考えます（[注釈1](#04-basics-of-differential-note1)）。
...
![微分2](../figures/source/04-basics-of-differential/03_03.png)
...
![微分3](../figures/source/04-basics-of-differential/03_03_02.png)
...
つまり、関数の最小値をとる点を求める問題では、接線の傾きが $0$ となる点が答えの候補（[注釈2](#04-basics-of-differential-note2)）となることがわかります。
...
![2点間を通る直線](../figures/source/04-basics-of-differential/03_04.png)
...
![1点での接線](../figures/source/04-basics-of-differential/03_05.png)

2点を通る直線の傾きの式と極限を組み合わせて、接線の傾きを求めることができます。

![1点での接線2](../figures/source/04-basics-of-differential/03_06.png)
...
今変数は $x$, $u$ の2つあるため、 $(\cdot)'$ という表記では、 $x$ で微分しているのか $u$ で微分しているのかの区別がつきません。
...
と表します。微分を意味する記号が、 $d$ から $\partial$ に変わっています。こうすると、 $\frac{\partial}{\partial x_m}$ は $x_m$ 以外を定数と考え、 $x_m$ にのみ着目して微分を行うという意味となります（[注釈3](#04-basics-of-differential-note3)）。
...
### 注釈 1 {#04-basics-of-differential-note1}
