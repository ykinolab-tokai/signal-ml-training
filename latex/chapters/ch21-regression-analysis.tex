...
本章では、基礎的な機械学習手法として代表的な\textbf{単回帰分析}と\textbf{重回帰分析}の仕組みを、数式を用いて説明します。
...
機械学習手法は、\textbf{教師あり学習 (supervised learning)} 、\textbf{教師なし学習 (unsupervised learning)} 、\textbf{強化学習 (reinforcement learning)} に大別され、単回帰分析は教師あり学習に含まれます。
...
与えられた入力変数から、$10$ や $0.1$ といった実数値を予測する \textbf{回帰 (regression)} と、「赤ワイン」、「白ワイン」といったカテゴリを予測する\textbf{分類 (classification)} の 2 つです。
...
\begin{itemize}
\item Step 1 : モデルを決める
\item Step 2 : 目的関数を決める
\item Step 3 : 最適なパラメータを求める
\end{itemize}
...
\subsection{Step 1：モデルを決める（単回帰分析）}


まずはじめに、入力変数 $x$ と出力変数 $y$ との関係をどのように定式化するかを決定します。

この定式化したものを \textbf{モデル} もしくは \textbf{数理モデル} と呼びます。
...
\StaticFigure{figures/source/21-regression-analysis/04_01.png}{部屋の広さと家賃}
...
ここで、$w$ は \textbf{重み (weight)} 、$b$ は \textbf{バイアス (bias)} の頭文字を採用しています。


\StaticFigure{figures/source/21-regression-analysis/04_02.png}{部屋の広さと家賃の関係}
...
パラメータで特徴づけられたモデルを用いる場合、与えられた \textbf{データセット} に適合するように最適なパラメータを求めることが目標となります。
...
と表すことができます（\hyperref[21-regression-analysis-note1]{注釈1}）。
...
次のステップに進む前に、データの\textbf{前処理 (preprocessing)} をひとつ紹介します。

データの \textbf{中心化 (centering)} は、データの平均値が 0 になるように全てのデータを平行移動する処理を言います。

下図は、データ集合 $(x_n, y_n) \ (n=1,\dots,11)$ を、平均が $(0, 0)$ になるように平行移動する例です。


\StaticFigure{figures/source/21-regression-analysis/04_03.png}{データの中心化}
...
\StaticFigure{figures/source/21-regression-analysis/04_04.png}{中心化によるパラメータ削減}
...
\subsection{Step 2：目的関数を決める（単回帰分析）}
...
今回は、目的関数として \href{https://tutorials.chainer.org/ja/03_Basic_Math_for_Machine_Learning.html#note2}{こちらの章} ですでに紹介した予測値と目標値との\textbf{二乗和誤差 (sum-of-squares error)} を用います。
...
\subsection{Step 3：最適なパラメータを求める（単回帰分析）}
...
\StaticFigure{figures/source/21-regression-analysis/04_05.png}{目的関数を最小化するパラメータ}


それでは、具体的に今回定めた目的関数 $L$ をパラメータである $w$ で微分してみましょう。

微分に関する基本的な計算や性質は \href{https://tutorials.chainer.org/ja/04_Basics_of_Differential.html}{こちらの章}で紹介しました。
...
ここで、微分の\textbf{線形性}から、和の微分は、微分の和となるため、
...
となっており、この部分は $t_n - wx_n$ と $(\cdot)^2$ という関数の\textbf{合成関数}になっています。
...
\StaticFigure{figures/source/21-regression-analysis/04_08.png}{家賃の予測値と目標値（中心化後）}
...
\subsection{Step 1：モデルを決める（重回帰分析）}
...
\subsection{Step 2：目的関数を決める（重回帰分析）}
...
このような行列を、\textbf{デザイン行列 (design matrix)} と呼びます。
...
\subsection{Step 3：パラメータを最適化する（重回帰分析）}
...
\StaticFigure{figures/source/21-regression-analysis/04_06.png}{目的関数の概形（2 次元）}


さらに、各次元が $w_1, w_2, L$ を表す 3 次元空間上においては、 $L$ の概形は下図のようになっています。


\StaticFigure{figures/source/21-regression-analysis/04_07.png}{目的関数の概形（3 次元）}
...
式変形の途中で理解できない部分があった場合は、\href{https://tutorials.chainer.org/ja/05_Basics_of_Linear_Algebra.html}{こちらの章} を読み返してみてください。
...
ここで、${\bf X}^{\rm T}{\bf X}$に\textbf{逆行列が存在すると仮定}して、両辺に左側から $({\bf X}^{\rm T}{\bf X})^{-1}$ を掛けると、
...
が導かれます。特に、この最後の式を\textbf{正規方程式 (normal equation)} と呼びます。
...
サンプルサイズ $N$ と独立変数の数 $M + 1$ が等しくない場合、${\bf X} \in \mathbb{R}^{N \times (M + 1)}$ は\textbf{正方行列ではない}ため、逆行列 ${\bf X}^{-1}$ を持ちません。
...
\subsection{注釈 1}
\label{21-regression-analysis-note1}
